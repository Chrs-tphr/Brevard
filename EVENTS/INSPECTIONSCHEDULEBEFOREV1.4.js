/*------------------------------------------------------------------------------------------------------/
| Program : InspectionScheduleBeforeV1.4.js  
| Event   : InspectionScheduleBeforeV1.4
|
...
| Action# : N/A
| 
| Notes   : - Roland VonSchoech, Accela - 7/29/2014 - Added custom and new functions for NOC Inspection Check.  Placed at bottom.  Use comment header/footer as guide to what was added.
|
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var currentUserGroup = aa.userright.getUserRight(appTypeArray[0],currentUserID).getOutput().getGroupName();
...
function getGISInfo(svc,layer,attributename)
	...
	// returns an array of associate arrays
		...
		if (refLicenseResult.getSuccess())
			...
			if (refArray)
				for (xxx in refArray)
					...
					tmpDate = refArray[xxx].getLicenseExpirationDate();
					if (tmpDate)
			...
			{ logDebug("**ERROR: Getting Licensed Professional Record.  Reason is: " + refLicenseResult.getErrorType() + ":" + gisObjResult.getErrorMessage()) ; return false }
		}	
...
function proximity(svc,layer,numDistance)  // optional: distanceType
	...
	for (a1 in fGisObj) // for each GIS object on the Cap
		...
		if (bufchk.getSuccess())
			...
			{ logDebug("**WARNING12:52 PM 06/20/2007: Retrieving Buffer Check Results.  Reason is: " + bufchk.getErrorType() + ":" + bufchk.getErrorMessage()) ; return false }	
...
function updateFee(fcode,fsched,fperiod,fqty,finvoice) // Updates an assessed fee with a new Qty.  If it is not found, adds it
	...
	if (!feeUpdated) // no existing fee, so add it
...
function createRefLicProfFromLicProf()
	...
	//
	var updating = false;
	refLicenseResult = aa.licenseScript.getRefLicensesProfByLicNbr(aa.getServiceProviderCode(),rlpId);
	if (!refLicenseResult.getSuccess())
		{ logDebug("**ERROR retrieving Ref Lic Profs : " + refLicenseResult.getErrorMessage()); return false; }
	else
		{
		newLicArray = refLicenseResult.getOutput();
		if (newLicArray)
			{ newLic = newLicArray[0] ; updating = true; logDebug("Updating existing Ref Lic Prof : " + rlpId);}
		else
	...
	if (myResult.getSuccess())
		...
		{ logDebug("**ERROR: can't create ref lic prof: " + createResult.getErrorMessage()); }
	}
...
function dateAddMonths(pDate, pMonths)
	...
	if ( totMonths > 0 )
		{
		//calc new year
		var addYears = ( totMonths==12 ? 0 : Math.floor(totMonths/12) );
...
function refLicProfGetAttribute(pLicNum, pAttributeName)
	...
	if (newLicArray)
		{
		newLic = newLicArray[0]; //use first ref license with this license #
...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	...
	if (arguments.length == 6) // License Number provided
		{	
		var refLicenseResult = aa.licenseScript.getRefLicensesProfByLicNbr(aa.getServiceProviderCode(),arguments[5]);
		if (!refLicenseResult.getSuccess())
			{ logDebug("**ERROR retrieving Ref Lic Profs : " + refLicenseResult.getErrorMessage());	return false; }
		else
			var refLicArr = refLicenseResult.getOutput();
		}
...
//=====================================	
// NOC Inspection Functions BELOW	
//=====================================	
	
//Return display message to cancel inspection, "" (empty string) to allow scheduling
function allowInspectionSchedulingForNOC()
{
	//Do not allow any inspections to be scheduled based on Record Status
	if(matches(capStatus, "Final", "Expired", "Void", "Withdrawn"))
	{
		logDebug("Prevent Inspection Schedule due to Record Status: " + capStatus);		
		return "An inspection cannot be scheduled on a Record with Status: " + capStatus;
	}
	
	//Do not allow any inspections to be scheduled based on Condition/Status
	if(appHasCondition("BUILDING", "Applied", "NOC required", null))
	{
		logDebug("Found 'NOC required' Condition.  Prevent Inspection Schedule.");
		return "An inspection cannot be scheduled until the recorded NOC has been submitted.";
	}
	
	if(inspType == "Building Final" && (appHasCondition(null, "Applied", "Drainage Survey", null) || appHasCondition(null, "Applied", "As Built Survey", null) || appHasCondition(null, "Applied", "As built survey", null) ) )
	{
		if(appHasCondition(null, "Applied", "Drainage Survey", null))
		{
			logDebug("Found 'Drainage Survey' Condition.  Prevent Inspection Schedule.");
			return "An inspection cannot be scheduled until the Drainage Survey has been submitted.";
		}
		else
		{
			logDebug("Found 'As Built Survey' Condition.  Prevent Inspection Schedule.");
			return "An inspection cannot be scheduled until the As Built Survey has been submitted.";
		}
	...
	if(isFinalInspection(inspType) && balanceDue > 0)
	{
		...
		var newBalanceDue = checkForMoratoriumFees();
		if(newBalanceDue > 0)
		{
			logDebug("There is a Balance Due.  Prevent Inspection Schedule.");
			return "An inspection cannot be scheduled until the outstanding fees are paid.";
		}
		else if(newBalanceDue == 0)
		{
			logDebug("Balance due is 0 after moratorium fee check.  Allow Inspection Schedule.");
			return "";
		}
			
	}
		
	if(inspType == "Tie Beam Lintel" && appHasCondition(null, "Applied", "Foundation Survey", null))
	{
		logDebug("Found 'Foundation Survey' Condition.  Prevent Inspection Schedule.");
		return "An inspection cannot be scheduled until the Foundation Survey has been submitted.";
	}
	
	if(inspType == "Framing" && appHasCondition(null, "Applied", "Foundation Survey", null))
	{
		logDebug("Found 'Foundation Survey' Condition.  Prevent Inspection Schedule.");
		return "An inspection cannot be scheduled until the Foundation Survey has been submitted.";
	}
	
	if(inspType == "Pre-lath" && appHasCondition(null, "Applied", "Foundation Survey", null))
	{
		logDebug("Found 'Foundation Survey' Condition.  Prevent Inspection Schedule.");
		return "An inspection cannot be scheduled until the Foundation Survey has been submitted.";
	}
	
	if(inspType == "Footing" && appHasCondition(null, "Applied", "Form Board Survey", null))
	{
		logDebug("Found 'Form Board Survey' Condition.  Prevent Inspection Schedule.");
		return "An inspection cannot be scheduled until the Form Board Survey has been submitted.";
	}
	
	//I only saw Concrete Deck and Pool Deck in the config I searched through.  Per customer email, use Pool Deck
	if(inspType == "Pool Deck" && appHasCondition(null, "Applied", "Form Board Survey", null)) 
	{
		logDebug("Found 'Form Board Survey' Condition.  Prevent Inspection Schedule.");
		return "An inspection cannot be scheduled until the Form Board Survey has been submitted.";
	}
	
	if(inspType == "Pre-Power" && balanceDue > 0)
	{
		//check for moratorium fees
		var newBalanceDue = checkForMoratoriumFees();
		if(newBalanceDue > 0)
	...
	//default
	return ""; //allow inspection

}

function checkForMoratoriumFees()
{
	//Function for Brevard.  Checks for a Moratorium Fee in a Fee Balance.
	//Return updated balanceDue (current balanceDue minus any moratorium fee balance found)
	//Use Return value to determine if inspection can be scheduled
	
	var newBalanceDue = 0;
	var moratoriumBalance = 0;
	var feeCodeBalance = 0;
		
	//Get all Fees on the Permit
	var arrRecordFees = loadFees();
	
	logDebug("About to loop through all Fees on the Record");
	for(fi in arrRecordFees)
	{
			var currentFeeCode = arrRecordFees[fi]["code"];
			feeCodeBalance = feeBalance(currentFeeCode);
			//logDebug("Current balance for Fee Code: " + currentFeeCode + " is " + feeCodeBalance);
			
			if(feeCodeBalance > 0)
			{
				var permitIssueExpireDateString = lookup ("FEE MORATORIUM", currentFeeCode);
				
				if(permitIssueExpireDateString == undefined)
				{
					//Look at next fee
					continue;
				}
				...
				{
					logDebug("Found a Moratorium Fee Expire Date in the standard choice: " + permitIssueExpireDateString);
					logDebug("Attempt to retrieve the Permit Issued Date from Workflow (Permit Issuance -> Issued)");
				
					//We found a moratorium fee
					//Get the Permit Issuance Date from Workflow
					if(isTaskStatus("Permit Issuance", "Issued"))
					{
						var issuedDate = taskStatusDate("Permit Issuance");
						logDebug("Found Workflow Status Date for Permit Issuance -> Issued: " + issuedDate);
						
						//If issuedDate <= permitIssueExpireDateString, then add feeCodeBalance to moratoriumBalance
						var jsIssueDate = convertDate(issuedDate);
						var jsExpireDate = convertDate(permitIssueExpireDateString);
						
						if(jsIssueDate <= jsExpireDate)
						{
							moratoriumBalance += feeCodeBalance;
							logDebug("Updated moratoriumBalance with feeCodeBalance: " + feeCodeBalance + ".  New moratoriumBalance: " + moratoriumBalance);
						}
                                                 else
						{
							logDebug("The Permit Issued Date is past the Moratorium Expiration date for this fee.  Fee Balance is due.");
						}

					}
					else
					{
						logDebug("Permit Issuance task is not set to the status of Issued.  Cannot determine if this (or any) Fee in moratorium without Permit Issuance date.  Return original balanceDue.");
						return balanceDue;
					}
				}
	...
	//Return the balance due minus moratorium fee if found
	newBalanceDue = balanceDue - moratoriumBalance;
	logDebug("Original balanceDue: " + balanceDue + ".  Moratorium Balance: " + moratoriumBalance + ".  Return newBalanceDue: " + newBalanceDue);
...
function Fee() // Fee Object
	...
	function convertDate(thisDate)
	{
		//7/29/2014 - Update for Brevard, include util.ScriptDateTime.  Existing function uses dom.ScriptDateTime
