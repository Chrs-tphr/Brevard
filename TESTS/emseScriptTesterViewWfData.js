...
/* WTUA */  var eventName = "WorkflowTaskUpdateAfter"; wfTask = "Document Intake"; wfStatus = "Accepted for Document Review"; wfDateMMDDYYYY = "01/01/2016";
...
//INSERT TEST CODE START
	...
	if(workflowResult.getSuccess()){
		...
		for(i in wfObj){
			...
			var thisProcessCode = fTask.getProcessCode();
			var thisTaskDescription = fTask.getTaskDescription();
			var thisDisposition = fTask.getDisposition();
			var thisActiveFlag = fTask.getActiveFlag(); 
			if(thisProcessCode == "ADHOC_WORKFLOW" && thisTaskDescription == "Document Intake"){
				logDebug("--------------------------------------------------------------------");
				logDebug(" "+wfObjCount+" - thisProcessCode: "+thisProcessCode);
				logDebug(" "+wfObjCount+" - thisTaskDescription: "+thisTaskDescription);
				logDebug(" "+wfObjCount+" - thisDisposition: "+thisDisposition);
				var thisDispositionNote = fTask.getDispositionNote(); logDebug(" "+wfObjCount+" - thisDispositionNote: "+thisDispositionNote);
				logDebug(" "+wfObjCount+" - thisActiveFlag: "+thisActiveFlag);
				var thisStepNumber = fTask.getStepNumber(); logDebug(" "+wfObjCount+" - thisStepNumber: "+thisStepNumber);
				var thisProcessID = fTask.getProcessID(); logDebug(" "+wfObjCount+" - thisProcessID: "+thisProcessID);
				
				var thisCurrentTaskID = fTask.getCurrentTaskID(); logDebug(" "+wfObjCount+" - thisCurrentTaskID: "+thisCurrentTaskID);
//				var thisDispositionComment = fTask.getDispositionComment(); logDebug(" "+wfObjCount+" - thisDispositionComment: "+thisDispositionComment);
